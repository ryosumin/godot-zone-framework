...
func _ready():
  #$DragMovement2D.connect("drag_begun", Callable(self, "_on_picked_up"))
  #$DragMovement2D.connect("drag_ended", Callable(self, "_on_put_down"))
  super._ready()
  _load_icon()
  if type == ChessLogic.KING:
    game.connect("king_checked", Callable(self, "_king_checked"))
    game.connect("king_not_checked", Callable(self, "_king_not_checked"))
...
func _load_icon():
  ...
  icon += "_"
  icon += PIECE_MAP[type]
  $Sprite2D.texture = load("res://Examples/Chess/Images/" + icon + ".png")
...
func _king_checked(color):
  if color == is_white:
    $Sprite2D.material = load("res://Examples/Chess/Materials/outline.tres")
