...
@export_enum("Pointy", "Flat") var type:int : set = type_set
@export_enum("Hexagonal", "Rectangular") var shape:int = false: set = shape_set
@export_range(1, 1000000) var width:int = 3: set = width_set
@export_range(1, 1000000) var height:int = 3: set = height_set
@export_range(1, 1000000) var cell_size:int = 16: set = cell_size_set
@export var cell_margin:int = 2: set = cell_margin_set
...
func _draw():
  if Engine.is_editor_hint():
    ...
    for point in points:
      ...
      draw_string(font,position,str(point), 0, -1, 16, Color.BLACK)
